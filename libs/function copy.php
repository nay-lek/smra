...
function Get_date_person_age($vn4digit,$ageRange,$option){
    $month_2Digit = substr($vn4digit,2,2); 
    $year_2Digit  = substr($vn4digit,0,2);
    $year_4Digit = ('25'. substr($vn4digit, 0, 2))-543;
    $ageRange  = $ageRange. ' year' ;

   // echo $ageRange .'<br>';
   // echo ($month_2Digit*1) .'<br>';
   
    if(($month_2Digit*1)<=9){   // ช่วงเดื่อนเกินปีงบ
       // หาวันเกิดตามช่วงปี
       
        $date=date_create(($year_4Digit-1)."-01-02");
        date_add($date,date_interval_create_from_date_string('- '.$ageRange));
        $date_stat_birthdate =  date_format($date,"Y-m-d");

       // นับวันที่ห่างไป 11 เดือน 29 วัน    
        $date_end_birthdate_11month  = date_create($date_stat_birthdate);
        $date_end_birthdate =  date_format(date_add(date_add($date_end_birthdate_11month,date_interval_create_from_date_string("1 year")), date_interval_create_from_date_string("-1 day")),"Y-m-d");
        $date_birthdate = "'".$date_stat_birthdate ."' and  '".$date_end_birthdate."'" ;

        //วันที่รับบริการใชช่วงปีงบประมานที่เลือก
        $date_serv_start =  date_create($year_4Digit."-10-01");
        date_add($date_serv_start,date_interval_create_from_date_string('- 1 year'));
        $date_serv_start =  date_format($date_serv_start,"Y-m-d");

        $date_serv_end  = date_create($date_serv_start);
        $date_serv_end  = date_add(date_add($date_serv_end,date_interval_create_from_date_string("1 year")),date_interval_create_from_date_string("-1 day"));

        $date_serv_end =  date_format($date_serv_end,"Y-m-d");

        $date_serv =  "'".$date_serv_start."' and  '".$date_serv_end."'";    
 

    } else{

        //$ageRange  = ($ageRange). ' year' ;
        $date=date_create($year_4Digit."-01-02");
        date_add($date,date_interval_create_from_date_string('- '.$ageRange));
        $date_stat_birthdate =  date_format($date,"Y-m-d");
         // นับวันที่ห่างไป 11 เดือน 29 วัน 
        $date_end_birthdate_11month  = date_create($date_stat_birthdate);
        ...
        //วันที่รับบริการใชช่วงปีงบประมานที่เลือก
        $date_serv_start =  date_create($year_4Digit."-10-01");
        date_add($date_serv_start,date_interval_create_from_date_string('+ 0 year'));
        $date_serv_start =  date_format($date_serv_start,"Y-m-d");
                    ...
                    SELECT  'AN9973' as 'ERROR_CODE',(select ERROR_DETAIL from pk_err_code_chk where ERROR_CODE = 'AN9973'  ) as 'ERROR_DETAIL' ,ifnull(count(*),0) as CC    
                     from person_anc
                      ...
                      where person_anc.labour_hospcode = (SELECT hospitalcode from opdconfig) and  concat(RIGHT((DATE_FORMAT(person_anc.anc_register_date ,'%Y') +543),2),DATE_FORMAT(person_anc.anc_register_date ,'%m')) like '$vn4digit'
                        ...
                        select 'AN9991' as 'ERROR_CODE',(select ERROR_DETAIL from pk_err_code_chk where ERROR_CODE = 'AN9991'  ) as 'ERROR_DETAIL' ,ifnull(count(*),0) as CC  
                            from person_anc_service ps_service
                            left JOIN person_anc ps_anc on ps_service.person_anc_id = ps_anc.person_anc_id
                            LEFT JOIN vn_stat on ps_service.vn = vn_stat.vn
                            left join patient on (select cid from person where person_id = ps_anc.person_id) = patient.cid
                            where   ps_service.anc_service_type_id = 1 and vn_stat.pdx not in('Z348','Z340') and ps_service.vn like'$vn4digit%'
...
function get_row_Dental_Error_all($vn4digit) {
    ...
    $dateserv_3y = explode("and",Get_date_person_age($vn4digit,3,'vist'));
    $dateserv_6y = explode("and",Get_date_person_age($vn4digit,6,'vist'));
    $dateserv_9y = explode("and",Get_date_person_age($vn4digit,9,'vist'));
    ...
    where  (dental_care_service_place_type_id not in(SELECT dental_care_service_place_type_id from dental_care_service_place_type) or dental_care_service_place_type_id is NULL or dental_care_service_place_type_id = '')
     ...
     select 'DT9204' as 'ERROR_CODE',(select ERROR_DETAIL from pk_err_code_chk where ERROR_CODE = 'DT9204'  ) as 'ERROR_DETAIL' ,ifnull(count(*),0) as CC  
     from  person
     LEFT JOIN person_anc on person_anc.person_id = person.person_id
     left join (SELECT ovst.hn  from dental_care
     LEFT JOIN ovst on dental_care.vn = ovst.vn and ovst.vstdate BETWEEN ".Get_date_person_age($vn4digit,3,'vist')." 
     ) person_dental on person.patient_hn= person_dental.hn
     WHERE person_dental.hn is null and person_anc.discharge <>'Y' and person_anc.labor_date is null 
 
    UNION

     select 'DT9300' as 'ERROR_CODE',(select ERROR_DETAIL from pk_err_code_chk where ERROR_CODE = 'DT9300'  ) as 'ERROR_DETAIL' ,ifnull(count(ps.cid),0) as CC
     from person ps  
      JOIN (( SELECT 'G3Age' as 'items' , person.person_id 
                      ...
                      , if((SELECT max(ovst.vstdate) 
                                     ...
                                     where ovst.vstdate BETWEEN  ".Get_date_person_age($vn4digit,3,'vist')."
                                     and  ovst.hn = person.patient_hn 
                                     GROUP BY ovst.hn 
                                 ) >=  DATE_ADD(person.birthdate,INTERVAL 4 year),'Black' ,
                                 ...
                                 if((SELECT max(ovst.vstdate) 
                                     ...
                                     and  ovst.hn = person.patient_hn 
                                     GROUP BY ovst.hn 
                                      ) <  DATE_ADD(person.birthdate,INTERVAL 4 year) , 'Green' ,
                                             if(datediff(DATE_ADD(person.birthdate,INTERVAL 4 year),now()) BETWEEN 31 and 90 , 'Yelow' , 
                                                 if(datediff(DATE_ADD(person.birthdate,INTERVAL 4 year),now()) BETWEEN 1 and 30 , 'Red' , 
                                                     if(datediff(DATE_ADD(person.birthdate,INTERVAL 4 year),now()) < 1 , 'Black' ,  
                                         '')))))  as 'chk_color'
                         , datediff(DATE_ADD(person.birthdate,INTERVAL 4 year),now()) 	as 'date_remine_new'  
                         , DATE_ADD(person.birthdate,INTERVAL 4 year) as 'Add_1year'
                      ...
                      , if((SELECT max(ovst.vstdate) 
                           ...
                           and  ovst.hn = person.patient_hn 
                           GROUP BY ovst.hn 
                      ) is null ,  datediff(".$dateserv_3y[1]."  , now()) , null)  as 'date_remine' 
                      ,if((SELECT max(ovst.vstdate) 
                           from dental_care 
                           INNER JOIN ovst on dental_care.vn  =  ovst.vn 
                           where ovst.vstdate BETWEEN ".Get_date_person_age($vn4digit,3,'vist')." 
                           ...
                           GROUP BY ovst.hn 
                                           ...
                                           datediff((SELECT max(ovst.vstdate) 
                                               ...
                                               and  ovst.hn = person.patient_hn 
                                               GROUP BY ovst.hn 
                                           ) , ".$dateserv_3y[0]." )) as 'count_DateSev2DateVisit' 
                      from person 
                      where person.birthdate BETWEEN ".Get_date_person_age($vn4digit,3,'DOB')." 
             ...
             select 'DT9301' as 'ERROR_CODE',(select ERROR_DETAIL from pk_err_code_chk where ERROR_CODE = 'DT9301'  ) as 'ERROR_DETAIL' ,ifnull(count(ps.cid),0) as CC
             from person ps  
             JOIN (( SELECT 'G6Age' as 'items' , person.person_id 
                                     ,PtName(person.patient_hn,person.cid) as pt_name,person.cid 
                                     , person.birthdate 
                                     , if((SELECT max(ovst.vstdate) 
                                            from dental_care 
                                            INNER JOIN ovst on dental_care.vn  =  ovst.vn 
                                            where ovst.vstdate BETWEEN  ".Get_date_person_age($vn4digit,6,'vist')." 
                                            and  ovst.hn = person.patient_hn 
                                            GROUP BY ovst.hn 
                                        ) >=  DATE_ADD(person.birthdate,INTERVAL 7 year),'Black' ,                            
                                            if((SELECT max(ovst.vstdate) 
                                                                        from dental_care 
                                                                        INNER JOIN ovst on dental_care.vn  =  ovst.vn 
                                                                        where ovst.vstdate BETWEEN  ".Get_date_person_age($vn4digit,6,'vist')." 
                                                                        and  ovst.hn = person.patient_hn 
                                                                        GROUP BY ovst.hn 
                                                                    ) <  DATE_ADD(person.birthdate,INTERVAL 7 year) , 'Green' ,
                                                            if(datediff(DATE_ADD(person.birthdate,INTERVAL 7 year),now()) BETWEEN 31 and 90 , 'Yelow' , 
                                                                        if(datediff(DATE_ADD(person.birthdate,INTERVAL 7 year),now()) BETWEEN 1 and 30 , 'Red' , 
                                                        if(datediff(DATE_ADD(person.birthdate,INTERVAL 7 year),now()) < 1 , 'Black' ,  
                                                    '')))))  as 'chk_color'
                                    , datediff(DATE_ADD(person.birthdate,INTERVAL 7 year),now()) 	as 'date_remine_new'  
                                    , DATE_ADD(person.birthdate,INTERVAL 7 year) as 'Add_1year' 
                                     , (SELECT count(DISTINCT(ovst.vn)) from dental_care  
                                             INNER JOIN ovst on dental_care.vn = ovst.vn 
                                             where ovst.vstdate   BETWEEN ".Get_date_person_age($vn4digit,6,'vist')."  
                                          and  ovst.hn = person.patient_hn  
                                     ...
                                     , if((SELECT max(ovst.vstdate) 
                                          ...
                                          and  ovst.hn = person.patient_hn 
                                          GROUP BY ovst.hn 
                                     ) is null ,  datediff(".$dateserv_6y[1]."  , now()) , null)  as 'date_remine' 
                                     ,if((SELECT max(ovst.vstdate) 
                                          from dental_care 
                                          INNER JOIN ovst on dental_care.vn  =  ovst.vn 
                                          where ovst.vstdate BETWEEN ".Get_date_person_age($vn4digit,6,'vist')." 
                                          ...
                                          GROUP BY ovst.hn 
                                                          ...
                                                          datediff((SELECT max(ovst.vstdate) 
                                                              ...
                                                              and  ovst.hn = person.patient_hn 
                                                              GROUP BY ovst.hn 
                                                          ) , ".$dateserv_6y[0]." )) as 'count_DateSev2DateVisit' 
                                     from person 
                                     where person.birthdate BETWEEN ".Get_date_person_age($vn4digit,6,'DOB')." 
                                     and person.house_regist_type_id in(1,3)  
                                     GROUP BY person.cid                                                      
                                        HAVING chk_color in('red','Yelow','Black') || chk_visit_date_service is null  
            ...
            select 'DT9302' as 'ERROR_CODE',(select ERROR_DETAIL from pk_err_code_chk where ERROR_CODE = 'DT9302'  ) as 'ERROR_DETAIL' ,ifnull(count(ps.cid),0) as CC			
            from person ps  
            JOIN (( SELECT 'G9Age' as 'items' , person.person_id 
                                    ,PtName(person.patient_hn,person.cid) as pt_name,person.cid 
                                    , person.birthdate 
                                    , if((SELECT max(ovst.vstdate) 
                                            from dental_care 
                                            INNER JOIN ovst on dental_care.vn  =  ovst.vn 
                                            where ovst.vstdate BETWEEN  ".Get_date_person_age($vn4digit,9,'vist')." 
                                            and  ovst.hn = person.patient_hn 
                                            GROUP BY ovst.hn 
                                            ) >=  DATE_ADD(person.birthdate,INTERVAL 10 year),'Black' ,                            
                                        if((SELECT max(ovst.vstdate) 
                                                                    from dental_care 
                                                                    INNER JOIN ovst on dental_care.vn  =  ovst.vn 
                                                                    where ovst.vstdate BETWEEN ".Get_date_person_age($vn4digit,9,'vist')." 
                                                                    and  ovst.hn = person.patient_hn 
                                                                    GROUP BY ovst.hn 
                                                                ) <  DATE_ADD(person.birthdate,INTERVAL 10 year) , 'Green' ,
                                                        if(datediff(DATE_ADD(person.birthdate,INTERVAL 10 year),now()) BETWEEN 31 and 90 , 'Yelow' , 
                                                                    if(datediff(DATE_ADD(person.birthdate,INTERVAL 10 year),now()) BETWEEN 1 and 30 , 'Red' , 
                                                    if(datediff(DATE_ADD(person.birthdate,INTERVAL 10 year),now()) < 1 , 'Black' ,  
                                                    '')))))  as 'chk_color'
                                    , datediff(DATE_ADD(person.birthdate,INTERVAL 10 year),now()) 	as 'date_remine_new'  
                                    , DATE_ADD(person.birthdate,INTERVAL 10 year) as 'Add_1year'
                                    ,   (SELECT count(DISTINCT(ovst.vn)) from dental_care  
                                            INNER JOIN ovst on dental_care.vn = ovst.vn 
                                             where ovst.vstdate   BETWEEN ".Get_date_person_age($vn4digit,9,'vist')."  
                                                and  ovst.hn = person.patient_hn  
                                    ...
                                    , if((SELECT max(ovst.vstdate) 
                                         ...
                                         and  ovst.hn = person.patient_hn 
                                         GROUP BY ovst.hn 
                                    ) is null ,  datediff(".$dateserv_9y[1]."  , now()) , null)  as 'date_remine' 
                                    ,if((SELECT max(ovst.vstdate) 
                                         from dental_care 
                                         INNER JOIN ovst on dental_care.vn  =  ovst.vn 
                                         where ovst.vstdate BETWEEN ".Get_date_person_age($vn4digit,9,'vist')." 
                                         ...
                                         GROUP BY ovst.hn 
                                                         ...
                                                         datediff((SELECT max(ovst.vstdate) 
                                                             ...
                                                             and  ovst.hn = person.patient_hn 
                                                             GROUP BY ovst.hn 
                                                         ) , ".$dateserv_9y[0]." )) as 'count_DateSev2DateVisit' 
                                    from person 
                                    where person.birthdate BETWEEN ".Get_date_person_age($vn4digit,9,'DOB')." 
                                    and person.house_regist_type_id in(1,3)  
                                    GROUP BY person.cid 
                                    HAVING chk_color in('red','Yelow','Black') || chk_visit_date_service is null 
                                    ORDER BY birthdate
            ...
            select 'DT9303' as 'ERROR_CODE',(select ERROR_DETAIL from pk_err_code_chk where ERROR_CODE = 'DT9303'  ) as 'ERROR_DETAIL' ,ifnull(count(ps.cid),0) as CC			
            from person ps  
            JOIN (( SELECT 'G12Age' as 'items' , person.person_id 
                                    ...
                                    , if((SELECT max(ovst.vstdate) 
                                            from dental_care 
                                            INNER JOIN ovst on dental_care.vn  =  ovst.vn 
                                            where ovst.vstdate BETWEEN   ".Get_date_person_age($vn4digit,12,'vist')."
                                            and  ovst.hn = person.patient_hn 
                                            GROUP BY ovst.hn 
                                        ) >=  DATE_ADD(person.birthdate,INTERVAL 13 year),'Black' ,                                                            
                                            if((SELECT max(ovst.vstdate) 
                                                                        from dental_care 
                                                                        INNER JOIN ovst on dental_care.vn  =  ovst.vn 
                                                                        where ovst.vstdate BETWEEN  ".Get_date_person_age($vn4digit,12,'vist')."
                                                                        and  ovst.hn = person.patient_hn 
                                                                        GROUP BY ovst.hn 
                                                                    ) <  DATE_ADD(person.birthdate,INTERVAL 13 year) , 'Green' ,
                                                            if(datediff(DATE_ADD(person.birthdate,INTERVAL 13 year),now()) BETWEEN 31 and 90 , 'Yelow' , 
                                                                        if(datediff(DATE_ADD(person.birthdate,INTERVAL 13 year),now()) BETWEEN 1 and 30 , 'Red' , 
                                                        if(datediff(DATE_ADD(person.birthdate,INTERVAL 13 year),now()) < 1 , 'Black' ,  
                                                    '')))))  as 'chk_color'
                                    , datediff(DATE_ADD(person.birthdate,INTERVAL 13 year),now()) 	as 'date_remine_new'  
                                    , DATE_ADD(person.birthdate,INTERVAL 13 year) as 'Add_1year'
                                    , (SELECT count(DISTINCT(ovst.vn)) from dental_care  
                                            INNER JOIN ovst on dental_care.vn = ovst.vn 
                                            where ovst.vstdate   BETWEEN ".Get_date_person_age($vn4digit,12,'vist')."  
                                         and  ovst.hn = person.patient_hn  
                                    ...
                                    from person 
                                    where person.birthdate BETWEEN ".Get_date_person_age($vn4digit,12,'DOB')." 
                                    and person.house_regist_type_id in(1,3)  
                                    GROUP BY person.cid 
                                    HAVING chk_color in('red','Yelow','Black') || chk_visit_date_service is null 
                                    ORDER BY birthdate
                 ...
                 select 'DT9299' as 'ERROR_CODE',(select ERROR_DETAIL from pk_err_code_chk where ERROR_CODE = 'DT9299'  ) as 'ERROR_DETAIL' ,ifnull(count(ps.cid),0) as CC			
                 from patient ps  
                 join (SELECT DISTINCT(ovst.hn) hn,GROUP_CONCAT(ovst.vn),GROUP_CONCAT(ovst.vstdate)
                                from dental_care
                            INNER JOIN ovst on dental_care.vn = ovst.vn
                            where ovst.vstdate BETWEEN ".Get_date_person_age($vn4digit,12,'vist')." 
                            GROUP BY ovst.hn
                            HAVING count(ovst.hn) > 1 ) dental_chk on ps.hn = dental_chk.hn 
                     ...
                     from person ps  
                      JOIN (( SELECT 'G3Age' as 'items' , person.person_id 
                            ...
                            JOIN (( SELECT 'G12Age' as 'items' , person.person_id 
                                                    ...
                                                    and person.house_regist_type_id in(1,3)  
                                                    GROUP BY person.cid 
                                                    HAVING chk_color in('red','Yelow','Black') || chk_visit_date_service is null || chk_visit_date_service is null
                                                    ORDER BY birthdate
                                                    ) ) dental_chk on ps.cid = dental_chk.cid 
...
function get_rows_Dental_Error_detail_list($error_code, $vn4digit, $xls) {
    ...
    $th_anc = "";
    if( $error_code=='DT9204'){
        $th_anc = "<th>ครรภ์ที่</th>";
    }
    ...
    $tb = $tb_title . " 	<tr>
                                      ...
                                      ".$th_anc."
                                      <th>DATEServ</th>
                                      <th>เหลือวันที่ให้บริการจากวันเกิด ถึง วันนี้ (".get_date_show(date('Y-m-d')).")</th>
                                      <th>เหลือเวลาบันทึกก่อนสิ้นปีงบประมาน</th>
                                 </tr>
    ...
    if( $error_code=='DT9300' || $error_code=='DT9301' || $error_code=='DT9302' || $error_code=='DT9303' ){
        ...
        ps.birthdate, '' as count_date_service , chk_color
            ...
            if($error_code=='DT9299'){
                                     
                                      $sql1  = "select  patient.hn as hn,cid,concat(pname,' ',fname,' ',lname) as ptname ,dental_chk.vstdate as vstdate 
                                                        from patient
                                                    inner join  (SELECT DISTINCT(ovst.hn) as hn,GROUP_CONCAT(ovst.vn) as vn,GROUP_CONCAT(concat(DATE_FORMAT(ovst.vstdate,'%d-%m-'),DATE_FORMAT(ovst.vstdate,'%Y')+543)) as vstdate 
                                                                    from dental_care
                                                                INNER JOIN ovst on dental_care.vn = ovst.vn
                                                                where ovst.vstdate BETWEEN ".Get_date_person_age($vn4digit,$age_select,'vist')."
                                                                GROUP BY ovst.hn
                                                                HAVING count(ovst.hn) > 1 ) dental_chk  on  patient.hn = dental_chk.hn   "; 
                           $query2 = mysql_query($sql1);
            }else{      
                if($error_code=='DT9204'){
                        $sql1 = "select  person.patient_hn as hn,cid,concat(pname,' ',fname,' ',lname) as ptname ,person_anc.preg_no ,'' as vstdate 
                                    from  person
                                    LEFT JOIN person_anc on person_anc.person_id = person.person_id
                                    left join (SELECT ovst.hn   from dental_care
                                    LEFT JOIN ovst on dental_care.vn = ovst.vn and ovst.vstdate BETWEEN ".Get_date_person_age($vn4digit,$age_select,'vist')." 
                                    ) person_dental on person.patient_hn= person_dental.hn
                                    WHERE person_dental.hn is null and person_anc.discharge <>'Y' and person_anc.labor_date is null ";
                            $query2 = mysql_query($sql1);
   ...
   // echo $rows ;
    ...
    while ($result = mysql_fetch_array($query2)) {
        ...
        $td_anc = "";
        if( $error_code=='DT9300' || $error_code=='DT9301' || $error_code=='DT9302' || $error_code=='DT9303' ){
                    $DATEServ_start = get_date_show($result['vstdate_start']);
                    $DATEServ_end = get_date_show($result['vstdate_end']);
                    $DATEServ = '[ '.$DATEServ_start . ' ถึง '. $DATEServ_end . ' ]';
                    $count_dateservice = $result['count_date_service'];
                    $chk_color   = $result['chk_color'];
                    $DATECount= $result['day_count']; //รับค่าจำนวนวันที่เหลือนับจากวันเกิดจนถึงวันสิ้นปีงบ


                    if($chk_color == 'Black'){                    
                        $tr_style = " <tr style='color:#FFFFFF ;background-color:#340801 '>" ;
                    }elseif ($chk_color == 'Red'){
                        $tr_style = " <tr style='background-color:#FA5133'>" ;
                    }elseif ($chk_color  == 'Yelow'){
                        $tr_style = " <tr style='background-color:#E6FB10'>" ;
                    }else{
                        $tr_style  = "<tr>";
                    }
                    $origin = new DateTimeImmutable(str_replace("'",'',$dateserv_12y[1]));
                    $target = new DateTimeImmutable(date(date('Y-m-d')));
                    $interval = $target->diff($origin);
                    $diff  =  $interval->format('%a วัน');
                    if(($interval->format('%a'))<366){
                        $diff = $diff;
                    }else{
                        $diff = "<font style='color:red'> ยังไม่ถึงรอบการบันทึก</font>";
                    }
             
        }else{
            if($error_code=='DT9299'){
                $DATEServ = $result['vstdate'];
            }else{
                if($error_code=='DT9204'){
                    $preg_no = $result['preg_no'];
                    $td_anc = "<td>$preg_no</td>";  
                }else{
                    $DATEServ = get_date_show($result['vstdate']);
                }
